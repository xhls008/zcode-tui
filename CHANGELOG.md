...
- **ZCode 3.11.2 空回复**：其内核仍报 0.16.5，但 `prompt_completed` 已改为
  提前的任务接收通知；现在等待真实 `turn.completed`，不再截断首个或后续回合。
  `turn.failed` 正常走失败收尾，空正文成功回合也能结束。
- **前台取消**：V4 可用时使用官方 `stop` 命令，等待真实终止事件后才复用会话；
  不可用时保留 legacy 路径。取消超时关闭所属内核进程，保留恢复 ID，避免把后续
  输入发进尚未停止的运行时。

### 验证

- 新增提前完成通知的顺序回归和隔离的真实内核测试，本地模拟模型验证连续流式
  对话、V4 取消后续接、关闭/恢复；不访问用户凭据或付费模型。
- 3.11.2-6792 官方 deb 已校验 SHA-512。Linux 包仍缺少 `@zcode/tui`，公开 CLI
  help 与 3.9.1 相同，app-server 新增 `v4/attachment/previewSource`，无方法删除。

### 体验

- **Agent Inspector**：保留远端新增的打开即刷新、V4 子代理实时进度和最终摘要，
  并将子代理生命周期输出与主对话分离。
- **Slash 命令**：Enter 直接执行高亮项，Tab 仅补全；移除重复的 `/model` 注册。
- **结束状态连续布局**：最新一轮的不可变尾部保留在固定输入框上方，超出空间或
  进入下一轮后按稳定渲染行写入原生 scrollback；thinking 临时覆盖尾部预留但不会
  消耗它，结束后可立即回填。不清屏也不改写 Banner，同时保持长回复正常上滚。
- **Token 口径更清晰**：footer 使用一位小数显示千/百万 token，并将累计值标为
  `sess`，可直接区分当前上下文占用 `ctx` 与 `/usage` 的会话累计 total。
...
- **命名自定义主题**：配置文件支持一套或多套 `[[custom_themes]]`；每套可从
  十一种内置主题选择 `base`（默认 `dark`），并覆盖现有全部颜色 token，含
  `selection_fg`。`/theme list` 以 `(custom)` 标记列出，`/theme <名称>` 可即时
  切换、持久化并在重启后恢复。

### 重构

- 自定义主题作为 v0.6.5 注册表的动态扩展；配置选择、保存校验、palette 解析、
  `/theme` 校验与列表以及 help 文本继续从同一注册表派生，没有新增平行白名单。

### 健壮性

- 自定义主题名称限制为 1–32 个小写字母/数字片段及单连字符；内置/自定义名称
  冲突、未知 base 和非法色值均提供清晰诊断，只禁用错误条目，不阻止启动或改写
  配置。

### 验证

- 新增单/多自定义主题、base/default 回退、所有非法输入、动态注册表一致性、
  `/theme` 列表/切换、配置持久化及重启恢复测试；Rust 测试 159/159，格式、
  Clippy 与 build 全部通过。
...
- **色盲友好主题**：新增 Okabe-Ito `accessible` 深色主题，以 `#0072B2`
  为主强调蓝、`#E69F00` 为强调橙，并使用该色板的语义绿、朱红与紫红；复用
  现有 `/theme` 即时切换、配置持久化、自定义 token 与 `NO_COLOR` 行为。
- **选中前景 token**：十一套调色板均新增 `selection_fg`，也可在配置中覆盖，
  浅色和深色主题的选中行不再共享一处写死的深色前景。

### 重构

- **主题注册表收敛**：名称、显示别名和完整调色板集中到唯一注册表；配置解析、
  保存校验、`/theme` 列表与校验、帮助文本及 palette dispatch 全部从注册表派生，
  后续新增内置主题只需修改一处。

### 修复

- 浅色主题会压暗 syntect 的 dark-source 语法色，并提高 `light` /
  `solarized-light` 的 dim 与语义色对比度；欢迎图标改用主题 `frame`、`text`、
  `code_bg` token，不再写死深色 RGB。

### 验证

- 新增注册表一致性、未知主题拒绝、accessible 分发与持久化、selection 前景、
  浅色语法适配及欢迎图标主题化测试；Rust 测试 152/152，格式、Clippy 与
  build 全部通过。
...
- **清华紫 / 北大红主题**：在现有 `dark` / `light` 之外新增
  `tsinghua`（清华紫）与 `pku`（北大红）两套完整配色，可通过 `/theme`
  列出并即时切换，继续复用原有配置持久化、自定义颜色覆盖及无色模式。

### 文档

- README 中英文版补充四种内置主题及 `/theme` 用法，并更新 v0.6.3 版本信息。
...
- **官方 Logo 与自适应渲染**：欢迎框内保留原始 Z 图标，并加入以官方 SVG 为
  参考重制的黑白纯文本 ZCODE 字标；移除低分辨率 half-block 图片和 Kitty
  占位符，终端宽度不足时直接隐藏大字标。
  inline 历史通过 scrolling region 稀疏写入，窗口缩放不再把固化的行尾空格
  重排成异常空行。
- **消息排版**：用户消息恢复背景横条和 `›` 标识，历史条目只保留一处分隔
  间距；完成条目按时间顺序持续追加进 scrollback，不固定轮数或按高度重排。
  所有消息换行预留右侧安全列，Markdown 在逻辑段落、
  列表项和代码块之间留出空行，软换行保持连续。
- **阶段追加**：app-server 回合在工具开始时提交前一段正文，在工具完成时提交
  工具结果，并在回合结束时提交尾段；底部活动区只绘制 thinking 状态，已经进入
