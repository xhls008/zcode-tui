...
def command_complete_state(args: argparse.Namespace) -> None:
    ...
    try:
        archive_dir.relative_to(args.root.resolve())
    except ValueError as exc:
        raise WorkflowError("archive path must stay inside the project root") from exc
    required = [archive_dir / "spec.md", archive_dir / "task.md", archive_dir / "checklist.md", archive_dir / "evidence" / "verification-status.json"]
    ...
    if initial_section == "completed":
        with state_lock(args.root):
            ...
            entry.update({"archive_path": args.archive_path, "tag": args.tag, "merge_commit": args.merge_commit})
    ...
    with state_lock(args.root):
        ...
        write_json(paths(args.root)[2], archive)
        write_json(paths(args.root)[1], queue)
    print(json.dumps({"status": "completed", "id": args.id, "archive_path": args.archive_path, "tag": args.tag}, ensure_ascii=False))
