...
## 隐私说明：使用内核，不启动桌面端的仓库快照上传链

**你可以通过本项目继续使用 ZCode 的编程内核，而不必启动官方桌面应用。**
本项目的托管 CLI 路径直接运行 `resources/glm/zcode.cjs`，不启动桌面
`app.asar/out/host/index.js`，也没有接入下述桌面端仓库快照上传服务。
这是有代码依据的架构区别，不是“所有文件都不会外发”的安全保证。

**审计发现（2026-09-18）**：官方 Linux 包 **3.11.2-6792 / 3.12.3-7463**
的桌面 host 中存在完整的“工作区及 `.git` 采集 → 压缩 → 加密暂存 → OSS
上传”链路，并接入普通会话发送等流程；执行还需要账号令牌、服务端上传凭据等
条件。`.git` 内部文件被优先纳入，不能依靠普通文件的敏感文件名过滤保护提交
历史。已检查的两版 CLI 内核中，未发现这套快照上传实现。

**我们的立场很明确：反对并谴责任何未充分告知、未经明确授权的仓库后台采集。**
用户让 AI 帮忙写代码，不应被默认为同意交出整个 Git 历史。对官方把仓库采集
嵌入普通会话的这一设计，我们提出明确批评：在已追踪的链路中，未见单独的上传
确认，也未见“优化体验”或“仓库索引”设置作为启停门禁。官方应提供清晰的采集
说明、默认关闭且可验证的独立开关，以及导出和删除渠道；“已经加密”不能代替
用户的知情与控制。这是对可核查设计的批评，不是对实际上传、用途或违法性的
未经证实的断言。

**放心使用需要知道边界：**

- **不用桌面上传链 ≠ 完全不联网。** 正常模型请求仍可能发送代码、diff、附件
  和工具输出；MCP、插件、shell 和 Browser Use 也可能联网。
- **本项目目前不是网络或文件隔离沙箱。** 工具审批不能代替操作系统级防外传；
  对敏感项目仍应限制可读目录、模型服务和网络出口。
- **本项目不能约束另行启动的官方桌面应用。** 结论针对已审计版本和本项目的
  托管 CLI 路径，不自动覆盖自定义启动程序、未来官方 TUI 或更新后的内核。
- **静态审计不证明谁的文件已经上传。** 3.12.3 的隐私审计不是该版
  所有功能兼容的保证；经典 CLI / 旧版流式运行基线见下表。

版本、包哈希、函数证据及复查方法见[隐私审计记录](docs/privacy-audit-2026-09-18.md)。
...
只读区分客户端成功记录、尝试、打包残留与证据不足，不触发上传；
**未发现证据不等于从未上传**。自定义目录、文件名清单及限制见[自查说明](docs/check-zhipu-upload.md)。

**后续回应：**[媒体报道](https://www.aizws.net/news/detail/12690)转述官方 9 月 18 日致歉并称已修复，
解释为初期默认开启的仓库索引/Wiki 功能，并宣布将开源、接受第三方审查。
本项目未独立验证修复范围或数据销毁；上述审计针对固定哈希的历史构件。

![到底在防谁？三格讽刺漫画：帮我改一行代码；放心，加密了；安全到你自己也打不开。](output/imagegen/privacy-comic.png)

*根据 [ferstar 的公开评论](https://blog.ferstar.org/posts/zcode-silent-workspace-snapshot-upload/)
中“连你自己和客户端本体都解不开”的讽刺点创作；对白为艺术化改编，不是网友原话或当前服务端状态证明。*

## 当前验证基线

| 组件 | 当前版本 / 状态 |
|---|---|
| ZCode Linux x64 桌面包 | **3.12.3-7463 / 3.11.2-6792**（官方 deb，SHA-512 已校验） |
| 官方 CLI kernel | **0.16.5**（3.12.3 / 3.11.2 / 3.9.1 同号，不代表同协议） |
| zcode-tui | **0.7.0** |
| 协议兼容 | **3.12.3：经典 CLI 创建/连续恢复；暂不支持 app-server/V4/会话内模型切换**；3.11.2：按真实 `turn.completed` 收尾 + V4 前台取消；3.9.1：0.16.5 runtime preferences + official MCP auth 安全回退 + legacy/V4；3.8.1/3.7.7/3.7.6：0.16.3 runtime preferences + legacy/V4；3.5.3：legacy + V4；3.3.6：legacy 控制路径 |
| 当前源码验证 | Rust 测试 185/185；Clippy 零告警；原生 build；11/11 Agent PTY；8 项浏览器运行时检查 + 4 项浏览器 PTY；另有 opt-in 真实内核/浏览器隔离测试 |
...
在 3.11.2 兼容验证时，官方无版本号 Linux feed 仍返回 3.9.1，官网已发布 3.11.2；更新检测显示的
“最新”仅指该 feed。3.11.2 校验使用官网链接的版本化 `linux-x64/latest.yml`，
没有绕过 SHA-512，也没有修改系统安装。

真实内核回归可用以下命令重跑（Node ≥ 22.5；临时 HOME + 本地模拟模型，无真实
凭据或付费模型请求；3.11.2 覆盖流式/V4/取消/恢复，3.12.3 覆盖经典 CLI 创建及连续恢复）：
...
- **Browser Use（已实测 ZCode 3.11.2）**：`--browser-use headless` 与可选
  `--browser-executable <绝对路径>` 明确走官方 `zcode --prompt`，支持启动前路径
  检测、工具进度、运行时错误诊断与取消。托管 wrapper 从同一官方包中补齐
  Playwright 运行时解析，不修改官方安装或下载 npm 包。默认显式使用 build 模式，
  不自动开放工具权限；此路径没有 token 流式、steer 或交互审批。
  使用前需重新运行 `install.sh` 更新 wrapper；见[使用与验证说明](docs/browser-use.md)。
